...
type PlayFn = (...args: unknown[]) => Promise<unknown>;

interface MuteCapable {
  mute: () => Promise<void>;
}
...
export function autoplayPolicyExtension(options: AutoplayPolicyExtensionOptions = {}): Extension {
  ...
  return {
    ...
    attach(player: YTEmbed): () => void {
      const cap = player as unknown as MuteCapable;
      let detached = false;
      let inFallbackAttempt = false;
      const activeTimers = new Set<ReturnType<typeof setTimeout>>();

      const restore = wrapInstanceMethod<PlayFn>(player, 'playVideo', (previous) => {
